...
stand <- make_hex_stand(0.5,1) |> subset(yp >= 0 & yp < 50) |> mutate(wtn = wt, stratid = NA)
...
ypmin <- min(plants$yp)
ypwid <- ypmax-ypmin
plants <- plants |> mutate(slice = case_when(yp < ypmin+ypwid/4 ~ 0,
                                   yp < ypmin+2*ypwid/4 ~ 1,
                                   yp < ypmin+3*ypwid/4 ~ 2,
                                   TRUE ~ 3))

crowns0 <- plants |> subset(slice == 0 & obj %in% 'crown') |> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.9), color=colormixer(color, "#D9F2FF", 0.9))
crowns1 <- plants |> subset(slice == 1 & obj %in% 'crown') |> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.7), color=colormixer(color, "#D9F2FF", 0.7))
crowns2 <- plants |> subset(slice == 2 & obj %in% 'crown')|> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.3), color=colormixer(color, "#D9F2FF", 0.3))
crowns3 <- plants |> subset(slice == 3 & obj %in% 'crown')

stems0 <- plants |> subset(slice == 0 & obj %in% 'stem')|> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.9), color=colormixer(color, "#D9F2FF", 0.9))
stems1 <- plants |> subset(slice == 1 & obj %in% 'stem')|> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.7), color=colormixer(color, "#D9F2FF", 0.7))
stems2 <- plants |> subset(slice == 2 & obj %in% 'stem')|> 
  mutate(fill=colormixer(fill, "#D9F2FF", 0.3), color=colormixer(color, "#D9F2FF", 0.3))
stems3 <- plants |> subset(slice == 3 & obj %in% 'stem')

plants2 <- rbind(crowns0,crowns1,crowns2,crowns3,stems0,stems1,stems2,stems3)
...
ggplot() +
  ...
  theme(legend.position = "none", 
        panel.background = element_rect(fill = rgb(0.85,0.95,1,0.3),
                                        colour = "black",
